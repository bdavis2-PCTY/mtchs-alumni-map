...
if ($a == "CityLocations"){
	...
	if ( $q->num_rows > 0 ){
		mail($email, "MTCHS Alumni Admin Login", "Click the following link to login to the MTCHS Alumni Administrator site: {$APP_PATH}/admin/AdminLogin.php?ref={$email} \n\nIf you did not request this login, just ignore this message.");
	...
	$message = "A new user has been requested in the database.\n\nUse the following link to confirm or deny this addition: {$APP_PATH}/confirmation.php?a=Verify&ID={$ID}";
	...
	$message = "A user has requested update in the database. Use the link below to confirm    {$APP_PATH}/confirmation.php?a=Update&Confirm=true&ID=".$ID."    or use the following link to decline     {$APP_PATH}/confirmation.php?a=Update&Confirm=false&ID=".$ID;
	mail($SMTP_OVERRIDE_EMAIL, $subject, $message);
	responseHandler($message);
